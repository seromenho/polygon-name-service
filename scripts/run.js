const main = async () => {
  ...
  try {
    // Test custom InvalidName error
    txn = await domainContract.register("notvalidleght", {
      value: hre.ethers.utils.parseEther("1234"),
    });
    await txn.wait();
  } catch (error) {
    console.log("Returned InvalidName custom error", error);
  }

  try {
    // Test custom AlreadyRegistered error
    txn = await domainContract.register("seromenho", {
      value: hre.ethers.utils.parseEther("1234"),
    });
    await txn.wait();
  } catch (error) {
    console.log("Returned AlreadyRegistered custom error", error);
  }
